...
let canvas;
let ctx;
let model; // loaded tfjs graph model
let alpha; // tensor with alpha values used to expand rgb to rgba
const t = {}; // object that will hold all created tensors
...
async function runInference(frame = 0) {
    ...
    t.resize = tf.image.resizeBilinear(t.pixels, options.resolution); // resize input
    ...
    t.result = model.execute(t.expand); // model execute
    ...
    if (!alpha)
        alpha = tf.ones([...options.resolution, 1], 'float32'); // create alpha channel tensor once
...
async function main() {
    ...
    await tf.ready();
    if (tf.env().flagRegistry['WEBGL_FORCE_F16_TEXTURES'])
        tf.env().set('WEBGL_FORCE_F16_TEXTURES', true);
    if (tf.env().flagRegistry['WEBGL_EXP_CONV'])
        tf.env().set('WEBGL_EXP_CONV', true); // <https://github.com/tensorflow/tfjs/issues/6678>
    if (tf.env().flagRegistry['USE_SETTIMEOUTCUSTOM'])
        tf.env().set('USE_SETTIMEOUTCUSTOM', true); // <https://github.com/tensorflow/tfjs/issues/6687>
